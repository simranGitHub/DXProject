...
node {
    ...
    def JWT_KEY_FILE = env.JWT_CRED_ID_DH
    ...
    withCredentials([file(credentialsId: JWT_KEY_FILE, variable: 'jwt_key_file')]) {
        stage('Create Scratch Org') {
            ...
            rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST_DH}"
